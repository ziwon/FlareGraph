...
import { captureNote, findClaims, followLinks, listLinksFor, neighborsForPath, readNote } from '../handlers.js';
...
async function callTool(env: Env, exec: SqlExec, name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'search_notes':
    ...
    case 'follow_links': {
      const notes = await followLinks(env, exec, String(args.path ?? ''), (args.limit as number) ?? 5);
    ...
    case 'compile_wiki_page':
      return compileWikiPage(env, exec, String(args.topic ?? ''), args.category ? String(args.category) : 'Concepts');
